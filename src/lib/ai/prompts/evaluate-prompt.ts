...
export function buildEvaluatePrompt(input: EvaluateInput): {
  ...
  const system = [
    ...
    'When a target job context is provided, calibrate expectations to that role and domain: flag missing points that matter for that job, and tie improvementSuggestions and betterAnswer to its stack and domain where relevant (e.g. "for a payments product, you\'d also mention…").',
    '',
  ...
  const jdBlock = input.jdContext
    ? `\n\nTarget job context (what the candidate is interviewing for):\n${input.jdContext}`
    : '';
